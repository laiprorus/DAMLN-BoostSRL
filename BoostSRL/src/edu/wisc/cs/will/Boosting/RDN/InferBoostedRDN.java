...
public class InferBoostedRDN {
    ...
    public double getF1ForEgs(List<RegressionRDNExample> examples, double threshold,
        ...
        Log.clearFile(scalingResultFile);
        Log.appendWriteln(scalingResultFile, "auc_roc: " + Utils.truncate(auc.getROC(), 6));
        Log.appendWriteln(scalingResultFile, "auc_pr: " + Utils.truncate(auc.getPR(), 6));
        Log.appendWriteln(scalingResultFile, "cll: " + Utils.truncate(auc.getCLL(), 6));
        ...
        if (threshold != -1) {
            ...
            if (writeQueryAndResults) {
                Utils.writeStringToFile(resultsString + "\n", new CondorFile(fileNameForResults));
            }
            return score.getF1();
        }
    ...
    private void writeToCollectorFile(List<RegressionRDNExample> examples) {
        ...
        try {
            ...
            if (collectorFile != null) {
                reportResultsToCollectorFile(collectBuffer, RunBoostedRDN.nameOfCurrentModel, null, null, 0.0, null);
            }
            for (RegressionRDNExample pex : examples) {
                ...
                if (collectorFile != null) {
                    reportResultsToCollectorFile(collectBuffer, RunBoostedRDN.nameOfCurrentModel, pex.isOriginalTruthValue() ? "pos" : "neg", prob, wgtOnEx, pex);
                }
            }
            ...
            if (collectorFile != null) {
                sortLinesInPredictedProbsFile(RunBoostedRDN.nameOfCurrentModel, fileName, fileNameSorted);
            }
        } catch (IOException e) {
...
//					Utils.println(regressionEx.provenance);
                ...
                if (writeAllResultsToCSVfile) {
                    sb.append("pos, " + prob + ", " + regressionEx + "\n");
			...
			/*	double egProb=prob;
                ...
                if (writeAllResultsToCSVfile) {
                    sb.append("neg, " + prob + ", " + regressionEx + "\n");
                }
