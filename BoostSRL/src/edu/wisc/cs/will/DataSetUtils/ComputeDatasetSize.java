...
public class ComputeDatasetSize {
	...
	public Map<String, Integer> getDomainSizes() {
		Map<String, Integer> domainSizes = new HashMap();

		for (Type type : getStringHandler().getKnownConstantsOfThisType().keySet()) {
			String domain = type.toString();
			int size = getStringHandler().getKnownConstantsOfThisType().get(type).size();
			domainSizes.put(domain,size);
		}
	...
	public static void main(String[] args) {
		...
		datasize.loadDirectory("C:/Users/MSI/Documents/code/BoostSRL/datasets/IMDB/test");

		Set<String> constants = datasize.getConstants();
		Map<String, Integer> domainSizes = datasize.getDomainSizes();
		Iterator<Literal> facts = datasize.getFacts();
		List<Example> posExamples = datasize.outerLooper.getPosExamples();

		Map<String, Set<Term>> domainConstants = new HashMap();

		for (Type type : datasize.getStringHandler().getKnownConstantsOfThisType().keySet()) {
			String domain = type.toString();
			Set<Term> typeConstants = datasize.getStringHandler().getKnownConstantsOfThisType().get(type);
			domainConstants.put(domain,typeConstants);
		}
